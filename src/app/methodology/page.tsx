...
export const metadata: Metadata = {
  ...
  description:
    "كيف تُحسب احتمالات تقدير: Dixon–Coles، Elo، Pi-ratings، الفورم، السياق، ثم الإرساء على السوق الحاد بوزن α وغربال المحسوم.",
};
...
const steps = [
  ...
  },
  {
    ...
    body: "مزج فورم 3 و5 و10 مباريات مع التسديدات/xG، وخصم إرهاق عند راحة أقل من 84 ساعة أو ازدحام ≥3 مباريات/7 أيام. وزنه يُتعلَّم مع بقية مكوّنات اللبّ لكل دوري (لا قفل ثابت).",
    color: "var(--warn)",
  },
  {
    n: "05",
    ...
    body: "بعد مزج DC/Pi/shots/true-xG تُطبَّق تعديلات محدودة (H2H، عشب، تكتيك مقيد، طقس، غيابات×قوة لاعب/XI، حكم) ثم تدخل 1X2 كمكوّن context. كل مضاعف قابل للإطفاء ويُختبر أثره بالـablation على الحزام التاريخي.",
    color: "var(--accent)",
  },
  {
    n: "06",
    role: "المرساة",
    titleAr: "السوق الحاد بعد اللبّ لا داخله",
    titleEn: "Sharp anchor · logit-pool α",
    body: "لبّ النموذج (pm) يُعاير بحرارة مُتعلَّمة ثم يُدمج مع Pinnacle/Betfair منزوع الهامش (Shin/Power حسب الدوري) بوزن α لكل دوري. α لا يُقبل إلا إذا أثبت bootstrap مقترن تفوّق الدمج على السوق؛ وإلا α = 0 ويُنشر السوق وحده. التقييم النهائي دائماً مقابل الإغلاق.",
    color: "var(--away)",
  },
];
    ...
    <div className="space-y-8">
        ...
        <div className="rounded-2xl border border-line bg-panel p-6 sm:p-8 space-y-4 shadow-2xs mt-2">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs sm:text-sm font-semibold text-muted leading-relaxed max-w-3xl">
              تُحسب التوقعات عبر لبّ إحصائي من خمس إشارات (أهداف، تصنيفات، قوة، فورم، سياق) يُعاير بحرارة مُتعلَّمة لكل دوري، ثم يُرسى على السوق الحاد منزوع الهامش بوزن α لا يُمنح إلا بدليل. الدقة القصوى الواقعية لـ1X2 نحو 50–55٪؛ الهدف إشارة أنظف وغربال صارم لا رقم أعلى.
            </p>
          </div>
      ...
      <SectionCard title="الفكرة والهدف الحسابي" subtitle="كيف نترجم البيانات الرياضية إلى احتمالات واقعية">
        <div className="p-5 sm:p-6 space-y-4 text-xs sm:text-sm font-semibold text-ink leading-relaxed">
          ...
          <p>
            تُبنى هذه الاحتمالات عبر لبّ من خمس إشارات تُحسب من بيانات رسمية مجانية (نتائج سابقة، تسديدات، xG)، يُعاير بحرارة مُتعلَّمة، ثم يُدمج مع السوق الحاد منزوع الهامش بوزن α يُقدَّر لكل دوري على مواسم كاملة. حين لا يتفوّق اللبّ على إغلاق السوق يكون α = 0: الاحتمال المنشور هو السوق نفسه، ويبقى دور النموذج في λ الأهداف والأسواق البديلة والغربال.
          </p>
          <p>
            <strong className="text-ink">غربال «المحسوم»</strong> طبقة قرار منفصلة لا تغيّر الاحتمالات: لا تُوصف مباراة بالمحسومة إلا باتفاق اللبّ والسوق على الجهة، واحتمال نهائي ≥ θ المعايرة لكل دوري (تُرفع فقط)، وفارق ≥ 20 نقطة عن ثاني نتيجة، ورأس تعادل مستقل منخفض، وبلا ديربي أو غياب ركيزة أو صاعد في أول 8 جولات أو عينة موسم أقل من 6 مباريات. كل ما عداه أرشيف «إشارة ضعيفة».
          </p>
          <p>
            <strong className="text-ink">النموذج 2</strong> يبقى طبقة ترتيب: يقيّم موثوقية ترشيح الفوز المباشر عبر ثلاثين عاملاً ويُرتّب المرشحين داخل ما اجتاز الغربال، بلا تغيير في 1X2.
          </p>
          <p>
            <strong className="text-ink">بيئة X2 لموسم 2025/26</strong> طبقة مقارنة لا تغيّر 1X2 المنشور: لكل دوري من السبعة يُحفظ توزيع الموسم (فوز المضيف، التعادل، فوز الضيف)، وX2 = التعادل + فوز الضيف. كتلة التعادل داخل Elo والفورم تُؤخذ من معدل تعادل ذلك الموسم. ثم يُقارن X2 لبّ النموذج بهذه البيئة وبسعر السوق منزوع الهامش. ارتفاع واضح عن البيئة مع سعر أضعف من النموذج يُعلَّم أخضر، والقرب من المعدل أصفر، والهبوط الواضح أو سعر أغلى من النموذج أحمر. معدل الدوري يصف البيئة التي تأتي منها المباراة، ولا يعني أن كل مباراة تحمل نفس X2.
          </p>
        </div>
      </SectionCard>

      <SectionCard title="القياس مقابل إغلاق السوق الحاد" subtitle="خط أساس واحد، ومقياسان منفصلان، ومسار زمني بثلاث لقطات">
        <div className="p-5 sm:p-6 space-y-3 text-xs sm:text-sm font-semibold text-ink leading-relaxed">
          <p>
            لكل توقع تُحفظ ثلاث لقطات: <strong>الإعلان</strong> (أول نشر)، <strong>التشكيلة</strong> (قبل الصافرة بساعة)، و<strong>الإغلاق</strong> (آخر سعر لدى Pinnacle أو بورصة Betfair). التقييم النهائي يُجرى مقابل الإغلاق منزوع الهامش بـBrier وLog-loss وRPS، ويُفصل إلى مقياسين لا يُخلطان: <strong>التغطية</strong> لكل المباريات و<strong>المحسوم</strong> لما اجتاز الغربال، حيث تُقارن نسبة الإصابة بالاحتمال المُعلَن لا برقم تسويقي.
          </p>
          <p>
            حزام walk-forward موسمي منذ 2022 يُقدِّر الأوزان والحرارة وα ورأس التعادل من مواسم سابقة فقط، ويختبر كل مضاعف سياقي بالإطفاء (ablation) وشبكة نصف عمر الاندثار. إذا هبطت إصابة شريحة المحسوم دون المُعلَن بأكثر من 5 نقاط تُرفع θ آلياً، وإذا هبطت دون 70٪ يعود الدوري إلى المراقبة حتى دليل جديد.
          </p>
        </div>
      </SectionCard>
      ...
      <SectionCard title="المعايرة والتقييم الزمني (Walk-Forward)" subtitle="ضمان عدم تسرب المستقبل واختبار صدق الاحتمالات">
        <div className="p-5 sm:p-6 space-y-3 text-xs sm:text-sm font-semibold text-ink leading-relaxed">
          <p>
            بعد خلط مكوّنات اللبّ (بما فيها context)، تُكيَّف حرارة Softmax المُتعلَّمة لكل دوري على مواسم كاملة لمنع الثقة الزائدة وتحسين Brier و Log-loss و RPS؛ ولا يُضرب بها أي معامل ضوضاء ثابت. الأوزان والحرارة تُستمدّ من الحزام التاريخي لا من نافذة قصيرة.
          </p>
          <p>
      ...
      <div className="rounded-2xl border border-amber-500/40 bg-surface overflow-hidden shadow-2xs">
        ...
        <div className="p-5 sm:p-6 space-y-3 text-xs sm:text-sm font-semibold text-ink leading-relaxed bg-surface">
          <ul className="space-y-2.5 list-disc list-inside text-muted">
            ...
            <li>
              <strong className="text-ink">الفرق الصاعدة وبداية الموسم:</strong> تُبذر الصاعدة من متوسط تقييمات الهابطين (لا من 1500 الأعمى)، وتنكمش تقييمات Elo/Pi نحو تقييم بداية الموسم خلال أول 8 جولات، ويُربط Dixon–Coles بأسبقيات الموسم السابق؛ ولا «محسوم» قبل 6 مباريات لكل طرف.
            </li>
            <li>
              <strong className="text-ink">لا تنبيهات رهان:</strong> الحافة مقابل السوق تُحسب للتشخيص فقط؛ لا +EV ولا حصص كيلي ولا ادعاء دقة 80٪ على كل المباريات.
            </li>
            <li>
